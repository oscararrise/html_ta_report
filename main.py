...
from utils import get_hired_people_current_year, get_open_job_applications
...
def main() -> None:
    ...
    job_titles = requisitions_df["title"].dropna().astype(str).str.strip().loc[lambda titles: titles.ne("")].drop_duplicates().tolist()
    # Get active applications for the open job titles.
    applications_df = get_open_job_applications(job_titles)
